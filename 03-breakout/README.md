...
Implementation of advanced game features including a dynamic Pause State, various Power-Ups using Abstract Factory, and shooting mechanics.
...
* **`F Key`**: Shoots bullets (when the Cannon power-up is active).
* **`Space`** *(Pause Key)*: Releases caught balls (if the Ball Catch power-up is active). If no balls are currently caught, it toggles the Pause State (freezes/resumes the game).
...
* **Power-Ups (Abstract Factory Pattern)**:
  ...
  * **CannonPower**: Equips the paddle with cannons to shoot bullets (`F Key`) that destroy bricks.
